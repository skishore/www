...
public class Board implements RepeatHandler{
	...
	private void resetBoard() {
	    if (gameMode == MULTIPLAYER) events.add(RESETBOARD);
	    ...
	    score = 0; combo = 0;
	    if (gameMode == MULTIPLAYER) attacks.clear();
	    boardState = RESET;
	}
	...
	private void getNextBlock() {getNextBlock(false); }
	...
	public void draw(Graphics g) {
	    if (boardState == PLAYING) {
	        ...
	        if (boardState == PAUSED) {
	            ...
	            if (gameMode == SINGLEPLAYER) paused.draw(g);
	            boardState++;
	        }
	...
	private void redrawBoard(Graphics g) {redrawBoard(g, false, RED); }
	private void redrawBoard(Graphics g, boolean tinted) {redrawBoard(g, tinted, RED); }
	private void redrawBoard(Graphics g, boolean tinted, Color tint) {
	    ...
	    drawGUI(g, tinted, tint);
	}
	
	private void drawGUI(Graphics g) {drawGUI(g, false, RED); }
	private void drawGUI(Graphics g, boolean tinted, Color tint) {
	    int listY, digit, i;
	    int x = 1;
	    ...
	    if (gameMode == SINGLEPLAYER) {
	        ...
	        int attack = j.next();
	        drawSmallBlock(g, blockData[attack+numBlocks], xOffset + 9*ATTACKSIZE/2, yOffset, ATTACKSIZE, LAMBDA);
	        int numDrawn = 1;

	        for (; j.hasNext();) {
	        	attack = j.next();
	            drawSmallBlock(g, blockData[attack+numBlocks], xOffset, yOffset, ATTACKSIZE, 3*LAMBDA);
	            numDrawn++;
	            ...
	            if (xOffset < squareWidth*NUMCOLS + 6*ATTACKSIZE) {
	                if (numDrawn == attacks.size()-1) {
	                    ...
	                    drawSmallBlock(g, blockData[10+numBlocks], xOffset, yOffset, ATTACKSIZE, 2*LAMBDA);
	                    drawSmallBlock(g, blockData[11+numBlocks], xOffset, yOffset, ATTACKSIZE, 2*LAMBDA);
	                }
	                break;
	            } 
	        }
	...
	private void drawBlock(Graphics g, Block block) {drawBlock(g, block, false, false, 0, 0, 0); }
	private void drawBlock(Graphics g, Block block, boolean erase, boolean shadow,
			int xOffset, int yOffset) {drawBlock(g, block, erase, shadow, xOffset, yOffset, 0); }
	private void drawBlock(Graphics g, Block block, boolean erase, boolean shadow, 
	            ...
	            if (shadow == false) {
	                if (erase == false) {
	                    drawSquare(g, point.x+xOffset, point.y+yOffset, 
	                    		mixedColor(WHITE, block.color, LAMBDA*(1-LAMBDA)));
	                } else {
	                    drawSquare(g, point.x+xOffset, point.y+yOffset, board[point.x+xOffset][point.y+yOffset]);
	                }
	            } else {
	                drawSquare(g, point.x+xOffset, point.y+yOffset, block.color, true);
	            }
	        }
	    }
	}
	...
	private void drawSmallBlock(Graphics g, Block block, int xOffset, int yOffset, int width, double lambda, 
			boolean tinted) {drawSmallBlock(g, block, xOffset, yOffset, width, lambda, tinted, RED); }
	private void drawSmallBlock(Graphics g, Block block, int xOffset, int yOffset, 
	    ...
	    for (int i = 0; i < block.numSquares; i++) {
	        ...
	        dim.y = width;
	        if (tinted == true) {
	        	g.setColor(mixedColor(tint, mixedColor(BLACK, block.color, lambda), 3*LAMBDA));
	            g.fillRect(pos.x, pos.y, dim.x, dim.y);
	        } else {
	        	g.setColor(mixedColor(BLACK, block.color, lambda));
	        	g.fillRect(pos.x, pos.y, dim.x, dim.y);
	        }
	    }
	}

	private void drawSquare(Graphics g, int x, int y, Color color) {drawSquare(g, x, y, color, false); }
